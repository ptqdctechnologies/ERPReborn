...
use App\Http\Controllers\ExportExcel\Process\ExportReportCreditNoteSummary;
...
use App\Services\Process\CreditNote\CreditNoteService;
use App\Services\WorkflowService;

class CreditNoteController extends Controller
{
    protected $creditNoteService, $workflowService;
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($response['metadata']['HTTPStatusCode'] !== 200) {
                throw new \Exception('Failed to fetch Create Credit Note');
            }
            ...
            //     throw new \Exception('Failed to fetch Submit Workflow Create Credit Note');
            // }
    ...
    public function UpdateCreditNote(Request $request)
    {
        try {
            ...
            if ($response['metadata']['HTTPStatusCode'] !== 200) {
                throw new \Exception('Failed to fetch Update Credit Note');
            }
            ...
            // if ($responseWorkflow['metadata']['HTTPStatusCode'] !== 200) {
            //     throw new \Exception('Failed to fetch Submit Workflow Revision Credit Note');
            // }
    ...
    public function ReportCNtoDN(Request $request)
    {
        try {
            $varAPIWebToken = Session::get('SessionLogin');
            $isSubmitButton = $request->session()->get('isButtonReportCNtoDNSubmit');

            $dataReport = $isSubmitButton ? $request->session()->get('dataReportCNtoDN', []) : [];
    ...
    public function ReportCNtoDNData($project_id, $site_id, $requester_id, $beneficiary_id, $project_name, $project_code, $site_code, $requester_name, $beneficiary_name, $site_name, $requester_position, $beneficiary_position)
    {
        try {
            ...
            Session::put("dataReportCNtoDN", $compact);

            return $compact;
        } catch (\Throwable $th) {
            Log::error("ReportCNtoDNData Error at " . $th->getMessage());
            return redirect()->back()->with('NotFound', 'Process Error');
        }
    }
    ...
    public function ReportCNtoDNStore(Request $request)
    {
        try {
            ...
            $compact = $this->ReportCNtoDNData($project_id, $site_id, $requester_id, $beneficiary_id, $project_name, $project_code, $site_code, $requester_name, $beneficiary_name, $site_name, $requester_position, $beneficiary_position);
    ...
    public function PrintExportReportCNtoDN(Request $request)
    {
        try {
            ...
            if ($dataReport) {
                if ($print_type === "PDF") {
                    $pdf = PDF::loadView('Process.CreditNote.Reports.ReportCNtoDN_pdf', ['dataReport' => $dataReport])->setPaper('a4', 'landscape');
                    $pdf->output();
                    $dom_pdf = $pdf->getDomPDF();
                    ...
                    return Excel::download(new ExportReportCNtoDN, 'Export Report Business Trip Request Summary.xlsx');
                }
            } else {
                return redirect()->route('CreditNote.ReportCNtoDN')->with('NotFound', 'Budget, Sub Budget, Requester, & Beneficiary Cannot Empty');
            }
        } catch (\Throwable $th) {
            Log::error("PrintExportReportCNtoDN Error at " . $th->getMessage());
            return redirect()->back()->with('NotFound', 'Process Error');
        }
    }
