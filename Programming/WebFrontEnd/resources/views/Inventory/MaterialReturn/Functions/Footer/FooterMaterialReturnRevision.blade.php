  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      $.each(data.DataAdvanceList, function(key, val2) {
        ...
        var Product = $("input[name='var_product_id[]']").map(function() {
          return $(this).val();
        }).get();
        var Quantity = $("input[name='var_quantity[]']").map(function() {
          return $(this).val();
        }).get();
        var RecordID = $("input[name='var_recordIDDetail[]']").map(function() {
          return $(this).val();
        }).get();
