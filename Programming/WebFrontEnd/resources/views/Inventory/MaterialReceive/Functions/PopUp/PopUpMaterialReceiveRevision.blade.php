<div class="modal fade" id="materialReceiveRevisionModal" tabindex="-1"
    aria-labelledby="materialReceiveRevisionModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="materialReceiveRevisionModalLabel"
            ...
            <div class="modal-body d-flex justify-content-center">
                <form id="editForm" method="POST" action="{{ route('MaterialReceive.RevisionMaterialReceiveIndex') }}">
                    @csrf
                    <div class="card mb-0" style="width: fit-content;">
                        <div class="card-body d-flex align-items-center justify-content-center" style="gap: 1rem;">
                            ...
                            <div class="form-group d-flex">
                                <div>
                                    <span id="modal_material_receive_document_number_icon"
                                        class="input-group-text form-control" data-toggle="modal"
                                        data-target="#myGetModalMaterialReceive"
                                        style="cursor:pointer; border-radius: 0;">
                                        <i class="fas fa-gift"></i>
                                    </span>
                                </div>
                                <div>
                                    <input id="modal_material_receive_document_number" class="form-control"
                                        style="border-radius:0; background-color: white;" readonly />
                                    <input id="modal_material_receive_id" class="form-control"
...
<script>
    $('#btn-edit').on('click', function () {
        const materialReceivedRefID = $('#modal_material_receive_id').val();

        if (materialReceivedRefID) {
            ShowLoading();
