<div class="modal fade" id="workRevisionModal" tabindex="-1" aria-labelledby="workRevisionModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="workRevisionModalLabel"
            ...
            <div class="modal-body d-flex justify-content-center">
                <form id="editForm" method="POST" action="{{ route('Work.revision') }}">
                    @csrf
                    <div class="card mb-0" style="width: fit-content;">
                        <div class="card-body d-flex align-items-center justify-content-center" style="gap: 1rem;">
                            ...
                            <div class="form-group d-flex">
                                <div>
                                    <span id="modal_work_document_number_icon" class="input-group-text form-control"
                                        data-toggle="modal" data-target="#myWorks"
                                        style="cursor:pointer; border-radius: 0;">
                                        <i class="fas fa-gift"></i>
                                    </span>
                                </div>
                                <div>
                                    <input id="modal_work_document_number" class="form-control"
                                        style="border-radius:0; background-color: white;" readonly />
                                    <input id="modal_work_id" class="form-control" name="modal_work_id"
...
<script>
    $('#btn-edit').on('click', function () {
        const workRefID = $('#modal_work_id').val();
        ...
        if (workRefID) {
            ...
            $('#modal_work_document_number').focus();
            $('#modal_work_document_number').css("border", "1px solid red");
        }
    });

    $('#btn-cancel').on('click', function () {
        $('#modal_work_id').val("");
        $('#modal_work_document_number').val("");
    });
</script>