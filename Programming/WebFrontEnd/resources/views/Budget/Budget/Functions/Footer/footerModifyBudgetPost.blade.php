<script>
    const budgetRefID               = {!! json_encode($budgetID ?? '') !!};
    const subBudgetRefID            = {!! json_encode($subBudgetID ?? '') !!};
    const currencySymbol            = {!! json_encode($currencySymbol ?? '') !!};
    const modifyBudgetListData      = {!! json_encode($modifyBudgetListData ?? []) !!};
    ...
    function showModifyBudgetListCard() {
        ...
        $.each(modifyBudgetListData, function(key, val2) {
            var html = 
                '<tr>' +
    ...
    function getBudgetDetail(currencyCode) {
        ...
        $.ajax({
            type: 'GET',
            url: '{!! route("getBudget") !!}?site_code=' + subBudgetRefID,
                ...
                if (data.message == "Invalid SQL Syntax") {
                    ...
                    if (currencyCode == "USD" || currencyCode == "EUR") {
                        $.each(dummy, function(key, val) {
                            ...
                            const findData      = modifyBudgetListData.find(value => value.productId == val.product_RefID);
                            ...
                            var html = 
                                '<tr>' +
                                    ...
                                    '<td class="container-tbody-tr-budget">' + '<div class="d-flex justify-content-center" data-toggle="tooltip" data-placement="top" title="Pesan"> <input style="border-radius:0; width: 55px !important;" class="form-control number-only" autocomplete="off" id="modify_budget_details" name="modify_budget_details" value="' + (findData?.modifyInput ? findData.modifyInput : '') + '"> </div>' + '</td>' +
                                    '<td class="container-tbody-tr-budget">' + '<div class="d-flex justify-content-center"> <input style="border-radius:0; width: 100px !important;" class="form-control number-without-negative" autocomplete="off" id="price_budget_details" name="price_budget_details" value="' + (findData?.priceInput ? findData.priceInput : '') + '"> </div>' + '</td>' +
                                    '<td class="container-tbody-tr-budget" style="padding-right: 0px !important;">' + '<div class="d-flex justify-content-center"> <input style="border-radius:0; width: 100px !important;" class="form-control number-only" autocomplete="off" id="total_budget_details" name="total_budget_details" value="' + (findData?.totalInput ? findData.totalInput : '') + '" disabled> </div>' + '</td>' +
                                '</tr>';
    ...
    document.getElementById('buttonBudgetDetails').addEventListener('click', function () {
        ...
        if (existingModifyBudgetListData) {
            try {
                modifiedBudgetListDatas = JSON.parse(existingModifyBudgetListData);
            } catch (error) {
                modifiedBudgetListDatas = [];
            }
        }
        ...
        if (updated) {
            ...
            const combinedData = [
                ...modifiedBudgetListDatas.reduce((map, obj) => map.set(obj.productId, obj), new Map()).values(),
            ];
    ...
    $(window).one('load', function(e) {
        ...
        showModifyBudgetListCard();
        getBudgetDetail(currencySymbol);
    });
</script>