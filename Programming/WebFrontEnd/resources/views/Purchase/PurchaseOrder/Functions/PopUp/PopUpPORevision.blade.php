<div class="modal fade" id="myPopUpPORevision" tabindex="-1" aria-labelledby="myPopUpPORevisionLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="myPopUpPORevisionLabel"
            ...
            <div class="modal-body d-flex justify-content-center">
                <form id="editForm" method="POST" action="{{ route('PurchaseOrder.RevisionPurchaseOrder') }}">
                    @csrf
                    <div class="card mb-0" style="width: fit-content;">
                        <div class="card-body d-flex align-items-center justify-content-center" style="gap: 1rem;">
                            ...
                            <div class="form-group d-flex">
                                <div>
                                    <span id="purchaseOrder_number_icon" class="input-group-text form-control"
                                        data-toggle="modal" data-target="#mySearchPO"
                                        style="cursor:pointer; border-radius: 0;">
                                        <i class="fas fa-gift"></i>
                                    </span>
                                </div>
                                <div>
                                    <input id="purchaseOrder_number" class="form-control"
                                        style="border-radius:0; background-color: white;" readonly />
                                    <input id="purchaseOrder_RefID" class="form-control" name="purchaseOrder_RefID"
...
<script>
    $('#btn-edit').on('click', function () {
        const purchaseOrderRefIDs = $('#purchaseOrder_RefID').val();

        if (purchaseOrderRefIDs) {
            ShowLoading();
