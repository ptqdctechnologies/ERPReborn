...
namespace App\Http\Controllers\Application\BackEnd\System\FileHandling\Engines\upload\stagingArea\localStorage\getFilesList\v1
    ...
    | ▪ Description : Menangani API fileHandling.upload.stagingArea.localStorage.getFilesList Version 1                            |
    +------------------------------------------------------------------------------------------------------------------------------+
    */
            ...
            //--->
             for ($i=0, $iMax=count($varDataReturn); $i!=$iMax; $i++)
                ...
                if(((bool) $varDataReturn[$i]['SignExistOnStorage']) == TRUE) {
                    $varDataReturn[$i]['Path'] = \App\Helpers\ZhtHelper\LocalStorage\Helper_LocalStorage::getBasePath($varUserSession).'Application/Upload/StagingArea/'.$varRotateLog_FileUploadStagingArea_RefRPK.'/'.$varDataReturn[$i]['Sys_RPK'];
                    }
                else {
