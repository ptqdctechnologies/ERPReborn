...
+----------------------------------------------------------------------------------------------------------------------------------+
*/
namespace App\Http\Controllers\Application\BackEnd\System\Transaction\Engines\update\humanResource\setBusinessTripTransportationCostTypeComponent\v1
    {
    /*
    +------------------------------------------------------------------------------------------------------------------------------+
    | ▪ Class Name  : setBusinessTripTransportationCostTypeComponent                                                               |
    | ▪ Description : Menangani API transaction.update.master.setBusinessTripTransportationCostTypeComponent Version 1             |
    +------------------------------------------------------------------------------------------------------------------------------+
    */
    class setBusinessTripTransportationCostTypeComponent extends \App\Http\Controllers\Controller
        ...
        */
        function main($varUserSession, $varData)
            ...
            try {
                $varSysDataProcess = \App\Helpers\ZhtHelper\Logger\Helper_SystemLog::setLogOutputMethodProcessHeader($varUserSession, __CLASS__, __FUNCTION__, 'Update Business Trip Transportation Cost Type Component Data (version 1)');
                try {
                    //-----[ MAIN CODE ]----------------------------------------------------------------------------( START POINT )-----
                    try {
                        if(!($varDataSend = \App\Helpers\ZhtHelper\System\BackEnd\Helper_API::getEngineDataSend_DataUpdate($varUserSession, (new \App\Models\Database\SchData_OLTP_HumanResource\TblBusinessTripTransportationCostTypeComponent())->setDataUpdate(
                            $varUserSession,
                            $varData['recordID'],
                            null,
                            null,
