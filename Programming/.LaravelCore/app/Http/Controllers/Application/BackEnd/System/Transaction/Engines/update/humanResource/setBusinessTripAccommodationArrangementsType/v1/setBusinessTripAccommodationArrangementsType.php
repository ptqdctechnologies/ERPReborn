...
+----------------------------------------------------------------------------------------------------------------------------------+
*/
namespace App\Http\Controllers\Application\BackEnd\System\Transaction\Engines\update\humanResource\setBusinessTripAccommodationArrangementsType\v1
    {
    /*
    +------------------------------------------------------------------------------------------------------------------------------+
    | ▪ Class Name  : setBusinessTripAccommodationArrangementsType                                                                 |
    | ▪ Description : Menangani API transaction.update.master.setBusinessTripAccommodationArrangementsType Version 1               |
    +------------------------------------------------------------------------------------------------------------------------------+
    */
    class setBusinessTripAccommodationArrangementsType extends \App\Http\Controllers\Controller
        ...
        */
        function main($varUserSession, $varData)
            ...
            try {
                $varSysDataProcess = \App\Helpers\ZhtHelper\Logger\Helper_SystemLog::setLogOutputMethodProcessHeader($varUserSession, __CLASS__, __FUNCTION__, 'Update Business Trip Accommodation Arrangements Type Data (version 1)');
                try {
                    //---- ( MAIN CODE ) ------------------------------------------------------------------------- [ START POINT ] -----
                    try{
                        if(!($varDataSend = \App\Helpers\ZhtHelper\System\BackEnd\Helper_API::getEngineDataSend_DataUpdate($varUserSession, (new \App\Models\Database\SchData_OLTP_HumanResource\TblBusinessTripAccommodationArrangementsType())->setDataUpdate(
                            $varUserSession,
                            $varData['recordID'],
                            null,
                            null,
