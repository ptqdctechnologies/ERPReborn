...
| ▪ Name Space : \App\Http\Controllers\Application\BackEnd\System\Report\Engines\PDF\dataList\master\getBusinessDocumentVersion\v1 |
|                                                                                                                                  |
| ▪ Copyleft 🄯 2021 Zheta (teguhpjs@gmail.com)                                                                                     |
+----------------------------------------------------------------------------------------------------------------------------------+
*/
namespace App\Http\Controllers\Application\BackEnd\System\Report\Engines\PDF\dataList\master\getBusinessDocumentVersion\v1
    {
    /*
    +------------------------------------------------------------------------------------------------------------------------------+
    | ▪ Class Name  : getBusinessDocumentVersion                                                                                   |
    | ▪ Description : Menangani API report.PDF.dataList.master.getBusinessDocumentVersion Version 1                                |
    +------------------------------------------------------------------------------------------------------------------------------+
    */
    class getBusinessDocumentVersion extends \App\Http\Controllers\Controller
        {
        /*
        +--------------------------------------------------------------------------------------------------------------------------+
        ...
        */
        function main($varUserSession, $varData)
            ...
            try {
                $varSysDataProcess = \App\Helpers\ZhtHelper\Logger\Helper_SystemLog::setLogOutputMethodProcessHeader($varUserSession, __CLASS__, __FUNCTION__, 'Get Business Document Version Data List PDF Report (version 1)');
                try {
                    ...
                    try{
                        if(!($varDataSend = $this->dataProcessing(
                            $varUserSession, 
                            [
                            'Title' => 'Business Document Version List',
                            'SubTitle' => [
                                (new \App\Models\Database\SchSysConfig\General())->getReferenceTextByReferenceID($varUserSession, $varData['parameter']['businessDocument_RefID'])
                                ]
                            ],
                            \App\Helpers\ZhtHelper\System\BackEnd\Helper_APICall::setCallAPIGateway(
                                ...
                                (\App\Helpers\ZhtHelper\System\BackEnd\Helper_API::getUserLoginSessionEntityByAPIWebToken($varUserSession))['APIWebToken'],
                                    ...
                                    'latest', 
                                    [
                                    'businessDocument_RefID' => $varData['parameter']['businessDocument_RefID'],
                                    'SQLStatement' => [
                                        'pick' => null,
                                        'sort' => null,
                                        'filter' => null,
                    ...
                    'Objects' =>
                        [
                            ...
                            'Cells' => [
                                ...
                                //[$varDataList[$i]['documentNumber'], 'L', 75],
                                [$varDataList[$i]['version'], 'C', 20],
                                [$varDataList[$i]['documentDateTimeTZ'], 'L', 35],
                                [$varDataList[$i]['annotation'], 'L', 95]
                                ]
                            ],
                        ]                    
                    ]
