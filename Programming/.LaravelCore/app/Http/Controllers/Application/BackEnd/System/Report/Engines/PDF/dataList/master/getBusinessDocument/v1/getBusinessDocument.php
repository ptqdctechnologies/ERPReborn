    ...
    class getBusinessDocument extends \App\Http\Controllers\Controller
        ...
        */
        function mainOLD($varUserSession, $varData)
            ...
            $varReturn = \App\Helpers\ZhtHelper\Logger\Helper_SystemLog::setLogOutputMethodHeader($varUserSession, null, __CLASS__, __FUNCTION__);
            try {
                $varSysDataProcess = \App\Helpers\ZhtHelper\Logger\Helper_SystemLog::setLogOutputMethodProcessHeader($varUserSession, __CLASS__, __FUNCTION__, 'Get Business Document Data List PDF Report (version 1)');
                try {
                    //-----[ MAIN CODE ]----------------------------------------------------------------------------( START POINT )-----
                    try{
                        if(!($varDataSend = $this->dataProcessing(
...
//                                    'parameter' => [
  //                                      'businessDocumentType_RefID' => $varData['parameter']['businessDocumentType_RefID']
    ...
    //                                    ],
        ...
        +--------------------------------------------------------------------------------------------------------------------------+
        */
        private function dataProcessingOLD($varUserSession, array $varDataHeader = null, array $varDataList = null, string $varQRCode = null)
            {
            $varRecordList_FirstPage = 41;
            $varRecordList_OtherPages = 52;
