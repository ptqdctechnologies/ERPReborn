...
| ▪ Name Space : \App\Http\Controllers\Application\BackEnd\System\Transaction\Engines\update\master\setWorkStructure\v1         |
|                                                                                                                                  |
| ▪ Copyleft 🄯 2026 Wisnu (wisnu.wirayuda01@gmail.com)                                                                               |
+----------------------------------------------------------------------------------------------------------------------------------+
*/
namespace App\Http\Controllers\Application\BackEnd\System\Transaction\Engines\update\master\setWorkStructure\v1 {
    ...
    | ▪ Description : Menangani API transaction.update.master.setWorkStructure Version 1                                        |
    +------------------------------------------------------------------------------------------------------------------------------+
    */
    class setWorkStructure extends Controller
    {
        ...
        */
        function main($varUserSession, $varData)
        {
            ...
            try {
                $varSysDataProcess = Helper_SystemLog::setLogOutputMethodProcessHeader($varUserSession, __CLASS__, __FUNCTION__, 'Update Supplier Category Data (version 1)');
                try {
                    ...
                    try {
                        if (
                            !($varDataSend =
                                Helper_API::getEngineDataSend_DataUpdate(
                                    $varUserSession,
                                    (new \App\Models\Database\SchData_OLTP_Master\TblWorkStructure())->setDataUpdate(
                                        $varUserSession,
                                        $varData['recordID'],
                                        null,
                                        null,
                        ...
                        $varReturn = Helper_API::getEngineDataSend_DataUpdateException($varUserSession, $ex);
                    }
                    //-----[ MAIN CODE ]------------------------------------------------------------------------------( END POINT )-----
