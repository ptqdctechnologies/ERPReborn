...
(function (factory) {
  ...
  var FC = ($.fullCalendar = { version: "0.0.0", internalApiVersion: 9 });
  ...
  function distributeHeight(els, availableHeight, shouldRedistribute) {
    ...
    els.each(function (i, el) {
      ...
      var naturalOffset = $(el).outerHeight(!0);
  ...
  function computeRangeAs(unit, start, end) {
    if (end != null) {
      return end.diff(start, unit, !0);
    } else if (moment.isDuration(start)) {
      return start.as(unit);
    } else {
      return start.end.diff(start.start, unit, !0);
    }
  }
  function divideRangeByDuration(start, end, dur) {
    ...
    if (Math.abs(months) >= 1 && isInt(months)) {
      return end.diff(start, "months", !0) / months;
    }
  ...
  function cloneRange(range) {
    return { start: range.start.clone(), end: range.end.clone() };
  }
  ...
  FC.moment.utc = function () {
    var mom = makeMoment(arguments, !0);
    if (mom.hasTime()) {
      mom.utc();
    }
    return mom;
  ...
  FC.moment.parseZone = function () {
    return makeMoment(arguments, !0, !0);
  };
    ...
    function buildFakeFormatString(chunks) {
      ...
      for (i = 0; i < chunks.length; i++) {
        ...
        if (typeof chunk === "string") {
          ...
          parts.push(
            MAYBE_MARKER + buildFakeFormatString(chunk.maybe) + MAYBE_MARKER
          );
        }
      }
  ...
  var RenderQueue = TaskQueue.extend({
    ...
    queue: function (taskFunc, namespace, type) {
      var task = { func: taskFunc, namespace: namespace, type: type };
      var waitMs;
      if (namespace) {
  ...
  var Popover = Class.extend(ListenerMixin, {
    ...
    position: function () {
      ...
      if (options.viewportConstrain !== !1) {
        top = Math.min(
          top,
  ...
  var DragListener = (FC.DragListener = Class.extend(ListenerMixin, {
    ...
    handleInteractionEnd: function (ev, isCancelled) {
      this.trigger("interactionEnd", ev, isCancelled || !1);
    },
    ...
    handleInteractionStart: function (ev) {
      ...
      if (ev) {
        ...
        if (subjectEl && this.options.subjectCenter) {
          if (this.origHit) {
            subjectRect =
              intersectRects(this.origHit, subjectRect) || subjectRect;
          }
      ...
      if (this.isFollowing && !this.isAnimating) {
        ...
        if (shouldRevert && revertDuration && !this.isHidden) {
          ...
          this.el.animate(
            ...
            { duration: revertDuration, complete: complete }
          );
        } else {
          complete();
        }
  ...
  var Grid = (FC.Grid = Class.extend(ListenerMixin, {
    ...
    buildDayClickListener: function () {
      ...
      dragListener.shouldCancelTouchScroll = !1;
      dragListener.scrollAlwaysKills = !0;
      return dragListener;
    },
  ...
  Grid.mixin({
    ...
    compareEventSegs: function (seg1, seg2) {
      return (
        seg1.eventStartMS - seg2.eventStartMS ||
  ...
  var DayTableMixin = (FC.DayTableMixin = {
    ...
    renderHeadDateCellHtml: function (date, colspan, otherAttrs) {
      ...
      if (this.rowCnt === 1) {
        classNames = classNames.concat(this.getDayClasses(date, !0));
      } else {
  ...
  var DayGrid = (FC.DayGrid = Grid.extend(DayTableMixin, {
    ...
    renderDates: function (isRigid) {
      ...
      this.rowCoordCache = new CoordCache({ els: this.rowEls, isVertical: !0 });
    ...
    renderBusinessHours: function () {
      var segs = this.buildBusinessHourSegs(!0);
  ...
  DayGrid.mixin({
    ...
    renderSegPopoverContent: function (row, col, segs) {
      ...
      var i;
      segs = this.renderFgSegEls(segs, !0);
    ...
    fgSegHtml: function (seg, disableResizing) {
      ...
      if (view.isMultiDayEvent(event)) {
        if (seg.isStart || seg.isEnd) {
          ...
          startTimeText = this.getEventTimeText(seg, null, !1);
        }
      } else {
        timeText = this.getEventTimeText(event);
        fullTimeText = this.getEventTimeText(event, "LT");
        startTimeText = this.getEventTimeText(event, null, !1);
      }
      return (
        '<a class="' +
        classes.join(" ") +
  ...
  var View = (FC.View = Model.extend({
    ...
    buildRenderQueue: function () {
      var _this = this;
      var renderQueue = new RenderQueue({ event: this.opt("eventRenderWait") });
    ...
    buildGotoAnchorHtml: function (gotoOptions, attrs, innerHtml) {
      ...
      date = FC.moment(date);
      finalOptions = { date: date.format("YYYY-MM-DD"), type: type || "day" };
    ...
    setDate: function (date) {
      ...
      var newDateProfile = this.buildDateProfile(date, null, !0);
      if (
        !currentDateProfile ||
        !isRangesEqual(
    ...
    addForcedScroll: function (scroll) {
      this.addScroll($.extend(scroll, { isForced: !0 }));
    },
  ...
  View.mixin({
    ...
    trimHiddenDays: function (inputRange) {
      return {
        ...
        end: this.skipHiddenDays(inputRange.end, -1, !0),
      };
    },
    currentRangeAs: function (unit) {
      var currentRange = this.currentRange;
      return currentRange.end.diff(currentRange.start, unit, !0);
    },
  ...
  var Calendar = (FC.Calendar = Class.extend(EmitterMixin, {
    ...
    changeView: function (viewName, dateOrRange) {
      if (dateOrRange) {
        if (dateOrRange.start && dateOrRange.end) {
          this.recordOptionOverrides({ visibleRange: dateOrRange });
        } else {
    ...
    pushLoading: function () {
      if (!this.loadingLevel++) {
        this.publiclyTrigger("loading", null, !0, this.view);
      }
    },
    popLoading: function () {
      if (!--this.loadingLevel) {
        this.publiclyTrigger("loading", null, !1, this.view);
      }
    },
  ...
  Calendar.mixin({
    ...
    queryToolbarsHeight: function () {
      return this.toolbarsManager.items.reduce(function (accumulator, toolbar) {
        var toolbarHeight = toolbar.el ? toolbar.el.outerHeight(!0) : 0;
  ...
  FC.datepickerLocale = function (localeCode, dpLocaleCode, dpOptions) {
    ...
    if ($.datepicker) {
      $.datepicker.regional[dpLocaleCode] = $.datepicker.regional[localeCode] =
        dpOptions;
  ...
  FC.sourceFetchers = [];
  var ajaxDefaults = { dataType: "json", cache: !1 };
  var eventGUID = 1;
  function EventManager() {
    ...
    function removeEventSources(matchInputs) {
      if (matchInputs == null) {
        removeSpecificEventSources(sources, !0);
      } else {
    ...
    function removeEvents(filter) {
      ...
      for (i = 0; i < sources.length; i++) {
        if ($.isArray(sources[i].events)) {
          sources[i].events = $.grep(sources[i].events, filter, !0);
        }
      }
      reportEventChange();
    }
  ...
  Calendar.prototype.isEventSpanAllowed = function (span, event) {
    ...
    return (
      ...
      (!eventAllowFunc || eventAllowFunc(span, event) !== !1)
    );
  };
  ...
  Calendar.prototype.isSelectionSpanAllowed = function (span) {
    ...
    return (
      ...
      (!selectAllowFunc || selectAllowFunc(span) !== !1)
    );
  };
  ...
  Calendar.prototype.computeBusinessHourEvents = function (wholeDay, input) {
    if (input === !0) {
      ...
      return this.expandBusinessHourEvents(wholeDay, input, !0);
    } else {
      return [];
    }
  };
  ...
  var BasicView = (FC.BasicView = View.extend({
    ...
    setGridHeight: function (height, isAuto) {
      if (isAuto) {
        ...
        distributeHeight(this.dayGrid.rowEls, height, !0);
      }
    },
    computeInitialDateScroll: function () {
      return { top: 0 };
  ...
  var MonthView = (FC.MonthView = BasicView.extend({
    buildRenderRange: function () {
      ...
      if (this.isFixedWeeks()) {
        rowCnt = Math.ceil(
          renderRange.end.diff(renderRange.start, "weeks", !0)
        );
  ...
  var YearView = (FC.YearView = View.extend({
    ...
    updateOptions: function () {
      ...
      this.yearColumns =
        parseInt(this.opt("yearColumns"), 10) || this.yearColumns;
    ...
    setGridHeight: function (grid, height, isAuto) {
      if (isAuto) {
        undistributeHeight(grid.rowEls);
      } else {
        distributeHeight(grid.rowEls, height, !0);
      }
    },
    hitsNeeded: function () {
      var i;
    ...
    render: function () {
      ...
      this.renderYear(monthsPerRow, weekCols, !0);
    },
    ...
    buildDayGrids: function () {
      ...
      var baseDate = view.intervalStart.clone().add(7, "days");
      view.dayGrids = [];
      $.each(nums, function (offset, m) {
        ...
        view.dayNumbersVisible = !0;
        dayGrid.numbersVisible = !0;
