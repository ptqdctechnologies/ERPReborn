...
class Users extends \Google\Service\Resource
{
   ...
   * help in this case. If `resolve_conflict_account` is set to `true`, the option
   * selected for [Find and add unmanaged
   * users](https://knowledge.workspace.google.com/admin/users/find-and-add-
   * unmanaged-users) will apply to resolve conflicting accounts: - A `200`
   * response code indicates the user was created (or replaced an existing
   * unmanaged personal account). - A `202` response code means that a conflicting
   * unmanaged personal account exists and was invited to join the organization. -
   * A `409` response code means that a conflicting account exists so the user
   * wasn't created (e.g. based on the option selected to preserve the account, or
   * if the email conflicts with an unmanaged work account or existing managed
   * user). For details on resolving duplicate account errors, see [Resolve
   * duplicate account errors](https://knowledge.workspace.google.com/p/duplicate-
   * account-errors) and [Transfer unmanaged work
   ...
   * @param array $optParams Optional parameters.
   *
   * @opt_param bool resolveConflictAccount Optional. Applies the option selected
   * for [Find and add unmanaged
   * users](https://knowledge.workspace.google.com/admin/users/find-and-add-
   * unmanaged-users) to resolve conflicting accounts when set to `true`. Default:
   * `false`
   * @return User
   * @throws \Google\Service\Exception
   */
