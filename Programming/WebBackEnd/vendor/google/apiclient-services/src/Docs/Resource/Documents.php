...
class Documents extends \Google\Service\Resource
{
   ...
   * content of the document's first tab populates the content fields in
   * `Document` excluding `Document.tabs`. If a document has only one tab, then
   * that tab is used to populate the document content. `Document.tabs` will be
   * empty. If you use a field mask that references the `Document.tabs` field (or
   * any subfield), the API implicitly treats the request as if you set
   * `include_tabs_content` to `true`.
